...
> - **Withheld** and **sending** are new confirmation outcomes on the submission, labelled in the admin: `src/cms/collections/form-submissions.ts`, with migrations `20260923_222518_form_confirmation_withheld` and `20260923_223657_form_confirmation_sending`, and `payload-types.ts`. Sending is the claim: a confirmation is counted from the moment it is claimed, and its record never says sent before it is. The first build marked the claim as sent, and the full suite caught a record reading sent while its alert was not yet recorded.
