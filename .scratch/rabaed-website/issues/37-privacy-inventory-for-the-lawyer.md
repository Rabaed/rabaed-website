...
**A test holds it to the forms.** `tests/unit/privacy-inventory.spec.ts` compares both documents against `src/forms/` and fails if a field is added, removed or renamed without them: the set of field names in each form's table must equal the definition's, in both languages, and every uploaded document's field must be named in the section about the private store. It opens no browser, as the screen mock registry's own two checks do not. Prose cannot be tested for being true, but the thing that actually goes wrong here — a field added, the inventory forgotten, the lawyer's policy quietly out of date — now fails a run. `src/forms/definition.ts` and the deployment guide both say so where a developer will be standing.

**What the two reviews changed, on 21 September 2026.**

- **The test moved to `tests/unit/`.** It opens no browser, and `tests/e2e` drives the built site. The spec's Testing Decisions named one seam and a second for pure calculation, and this is neither — so it now names a third: one file in the repository held to another, which no running application can observe. The screen mock registry's own two checks are the same shape and are named there with it; they stay beside the suite they belong to.
- **The rate limit was overstated.** Both documents said the site refuses a sixth *request* from one address within an hour. It counts stored submissions: a request turned away by the trap field or a missing one-time reference is never stored, so it is never counted. It is a cap on what one sender can store, not on how often they try, and both documents now say so.
- **The two consents are stored identically.** The Arabic implied «موافقة» and «إقرار» were two stored values; `CONSENT_RECORDED` is one literal, `accepted`, for both.
- **«الراعين» came out of section 02.** The policy's sharing clause names partners, affiliates and advertisers; sponsors appear in its separate clause about links to other sites, which is not about sharing.
- **Version history was missing.** Articles and case studies keep up to 50 earlier versions (`editorialVersions`), so a client's name or an attributed quotation taken off a published page is still in the database — exactly the personal data the note under that table flags. Form submissions are not versioned, and the documents say both.

**On "retention period stated for each category":** there is no retention period anywhere, and none was invented. Each category states what is true — kept until someone deletes it by hand, or, for the mailbox and the host, whatever they are set to, which nobody has written down. Section 11 asks the founders for a period rather than the document guessing one.
