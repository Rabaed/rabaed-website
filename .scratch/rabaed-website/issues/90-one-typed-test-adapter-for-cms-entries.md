...
>
>   When the test ends, pass or fail, each entry is put back. If what is published differs from what the test first read, that is published again; otherwise, a draft the test saved is discarded, as the suites did. It compares what is published rather than tracking the adapter's own saves, so a change published through the admin's own form (the start page's last test) is put back too.
>
...
> - **Discarding a draft by restoring the latest published version** blanked the English partnership form's settings when this was tried first: that global's oldest published version is an empty one. The ten suites' globals are not affected, and the adapter's suite uses the closing section instead.
