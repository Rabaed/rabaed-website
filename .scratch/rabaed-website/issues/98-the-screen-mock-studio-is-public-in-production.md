...
- `tests/e2e/studio-in-production.spec.ts` starts the first test server's build a second time with `VERCEL=1` and `VERCEL_ENV=production`, on `TEST_PORT + 4000` (refused if taken; `docs/agents/parallel-sessions.md` lists it) and with no database, and asks for studio pages in both languages, with a trailing slash, as the client router asks (`RSC: 1`) and with the prefix percent-encoded (all 404); then once more with `VERCEL_ENV=preview` (200, the mock's markup). The same build answering both ways is what shows the decision is made at request time.
- Not checked on a deployment, since that needs a merge: after this merges, `/studio/ar/correspondence` on the production site should answer 404, and on a preview 200.
