...
- [ ] A page's content module reads its page from the CMS in the locale asked for (`src/cms/pages.ts`, through the module in `src/content/pages/`), and refuses a locale the page is not published in — never another locale's words (ticket 52's `inLocale`, kept)
...
- **The start page's Trust strip got a switch too.** The logos stay ticket 20's; whether the strip shows on this page is a section like any other (spec: Content model).
- **Two things are not tested, and why.** The refusal of a language a page is not published in cannot be reached through the site until ticket 40 routes English pages; `src/cms/pages.ts` throws it. Restoring an old published version from the admin's Versions writes it back without Payload's checks, so a version published before a field was added could come back with that field empty; no page has changed its fields yet, so there is no such version today.
- **The page name from ticket 32 stays beside the search title and description** in `start.ts`, not in the CMS, since ticket 26 moves all three together.
