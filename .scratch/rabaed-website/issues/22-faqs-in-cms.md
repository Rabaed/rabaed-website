...
**Resolved (14 September 2026).** FAQs are the `faq-entries` collection (`src/cms/collections/faq-entries.ts`): one entry per question per language, on one of the five pages (`src/cms/faq-pages.ts`), imported by the migration `20260914_193520_import_faq_entries` from the frozen `src/migrations/faq-import/entries.ts`. Each page module completes its Questions section with `withQuestions` (`src/content/pages/page-content.ts`) — the section's id and the page's questions from `pageQuestions` (`src/cms/faqs.ts`) — and hands it to the one `Questions` section (`src/components/questions.tsx`). Decided here, rather than asked for:
...
- **Answers are plain text with two marks**, because a textarea cannot show formatting and rich text would be more than an answer needs: a Latin file name between backticks is set left to right in DM Mono, and `{payout}` / `{clientDiscount}` insert the Referral Program values, so ticket 52's "held once" still holds for the referral page's answer. An answer naming an unknown value, leaving a backtick open or putting Arabic between backticks (DM Mono has no Arabic) is refused. `answerText` turns an answer into what the page draws. The structured data itself is ticket 32's, so this box is met by giving it one source to read: ticket 32 now carries a note to build its FAQ data from the entries the section is handed, with `plainText`, never from the stored answer.
- **A reorder waits behind a draft.** Payload writes a moved question's new place to its newest version, so a question with an unpublished draft keeps its old place on the site until that draft is published. `docs/deployment.md` tells Editors so.
- **Limits:** a question 160 characters, an answer 800 — several times the longest the Reference site has — both refused beyond, and tested.
...
- **Tests:** `tests/e2e/faqs.spec.ts` publishes only an addition to the end of one referral answer, which no other suite can notice. Everything else it adds, hides, reorders or removes stays a draft and is checked in the editor's preview, because every page's words and pictures are being checked by other suites at the same moment. A question is added and hidden through the admin's own screens; the reorder is sent to the endpoint the admin's list calls when a row is dropped, and the removal through the API, rather than by dragging or clicking. A published hide or reorder reaching visitors is covered only by the refresh the collection's hook shares with the published edit.
