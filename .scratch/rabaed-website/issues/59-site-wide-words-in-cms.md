...
**From ticket 32 (15 September 2026).** Each inner page's breadcrumb structured data names the page with the same words as its menu entry, written out at the page for now: `'المنتج'` and `'ابدأ'` in `src/app/(ar)/product/page.tsx` and `start/page.tsx`, `'برنامج الإحالة'` and `'برنامج الشراكات'` in the referral and partnership pages, `'الرئيسية'` / `'Home'` in `src/components/structured-data.tsx`, and the blog's from `BLOG_COPY.eyebrow`. When the menu's labels move into the CMS, read those names from the same place, or renaming a page in the menu leaves its breadcrumb naming the old one. `tests/e2e/structured-data.spec.ts` restates the names it expects, so an intended rename updates it there too.
