...
**The Screen mocks are ticket 41's.** This ticket first proposed their descriptions too; ticket 41 merged first with its own English pictures and descriptions, so its proposal stands and this one's was taken out at the merge — the Screen mocks entry is read by the English home and product pages like any other shared entry. The schema migration was regenerated on top of ticket 41's under its own name, and its SQL is the same statement for statement.

**Left for later.** `llms.txt` stays Arabic, as its own comment says the site is: an English one is a question for when the English pages are published.
