...
- **Globals**: site settings (contact details, WhatsApp number, social links), the words every page shares — the header's menu, the footer and the not-found page, as one entry with a tab each (ticket 59) — the line under the blog's and the case studies' indexes, Trust strip logos (image, name, link, order), Referral Program values (amount, client discount).
- **A menu or footer link carries its words and its destination**, both an Editor's (ticket 59, the founder's decision of 20 September 2026): a page added later can be linked without a developer. The shape of a destination is checked — a path on the site, or a full `https://` address — but not whether it leads anywhere, since refusing an address that matches nothing would refuse the link this allows. A page the menu names is named the same way in the trail search results show, read from the menu, so renaming it renames both.
- **The header is full at the words the site launched with.** At 981px, the narrowest width that draws the menu as a row, four links and the Partnerships word carry eleven characters each beside a twelve-character sign-in link and a fifteen-character demo button — which is what those words measure today. An Editor rewords the menu within that room rather than adding to it; a longer menu is a design decision, not a CMS limit.
...
- **What an Editor can change on a page:** its words and images, the items in its lists (add, remove, reorder), and whether a section shows. Section order is fixed. Sections that links point at cannot be hidden: the demo form, the start page's FAQ, the tool page's download and how-it-works, the referral page's signup and how-it-works. Sections whose design is built around an exact count stay locked at it: the before/after steps (4), the calculator's sliders (3), the hero's buildings (3), the hero's statuses (4), which follow the document round the three buildings, each Record trail's steps (4), and the three parties. The founder locked the hero's statuses and the Record's steps on 15 September 2026 (ticket 58).
- **A Trust strip mark carries its file, the company's name, the height it is drawn at and, where the company gave one, an address** (ticket 20). The height is the Editor's because a wide wordmark must stand shorter than a compact monogram to look the same weight. A mark must be 88 pixels tall or more — twice the tallest the bar draws — or be a vector. An SVG is accepted and sanitised before anything is stored (ADR-0010).
...
**One seam, at the highest point: the running application, driven by Playwright.** Everything below it — components, CMS queries, form handlers, image pipeline — is exercised through that seam rather than tested directly. A second, narrow seam is permitted only for pure calculation with no I/O, where a direct unit test is cheaper and sharper than driving the site: the delay-cost formula, and the SVG sanitiser (ticket 20, ADR-0010), whose worth is the long list of things it must strip — a list far cheaper to state directly than to drive through an upload form one file at a time. That an upload reaches the sanitiser is asked through the one seam, as everything else is. The same seam holds what `npm run cms:rebase-migrations` decides about a branch's migrations — which are its own, what each is named, what a regenerated one keeps (ticket 88) — which no running site could observe at all. A third holds one file in the repository to another — the screen mock registry to the mocks on disk, the privacy inventory to the form definitions (ticket 37) — where what is asserted is the repository's own consistency, and no running application can observe it. A fourth loads the CMS configuration in a process of its own and asks what it did on starting, for what only a start the suite never makes can show: that the CMS starts without loading jsdom (ticket 65), and the database pool and address a Vercel deployment connects with (ticket 83). It asks the configuration and connects to nothing.
