...
The first paragraph under a heading, written so that it can be lifted and quoted on its own: a standalone answer of 30 to 60 words, understood without reading a word above it. An answer engine quotes a paragraph, not a page, so this is the paragraph that gets cited. Required of every article and case study since ticket 23, and of the four sections `reference/HANDOFF.md` §6.4 names since ticket 35; the rule itself is `src/cms/answer-first.ts`.
_Avoid_: lead, intro, summary (a summary is a different field, shown under the title on an index and in a search result).
