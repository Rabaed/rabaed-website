...
- **A replaced picture has only the crop an Editor uploads beside it** (ticket 79). The CMS's Screen mocks entry has a Phone crop upload next to each language's replacement picture, held to the crop's shape at the size the export makes it (1040×1300) or larger. On a phone each language shows its uploaded crop if there is one; otherwise the exported crop while the picture is not replaced; otherwise the replacement whole, with ticket 77's hint — never the exported crop of the screen it replaced. An uploaded crop opens that language's current whole screen, the replacement if there is one. Ticket 77's tests of the pan run against a replaced screen with no crop in a preview (`product-text.spec.ts`), because every screen visitors are sent today has a crop.
