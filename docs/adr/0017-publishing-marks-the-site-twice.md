...
- **Held by `tests/e2e/stale-render.spec.ts`,** which reproduces the race at the database rather than in the site's code: it locks the site settings, so a rebuild of `/tool` reads the tool page's words and then waits for the footer's; publishes new words while it waits; lets it go; checks the held rebuild answered with the words from before, so the race really was set up; and expects the new words to arrive. Without the second mark they never do. It runs on the test suite's second server, where suites run one at a time (`playwright.config.ts`, ticket 89), so no other suite's publish can land in its wait and rescue the page: in the full suite it is the proof as well as the guard. Until ticket 89 it ran beside other suites that publish, and only a run of it alone was the proof.
