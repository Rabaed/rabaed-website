...
export async function POST(request: NextRequest, context: RouteContext<'/api/forms/[formId]'>) {
  ...
  if (!definition) return Response.json({ outcome: 'refused', message: 'تعذّر إرسال النموذج.' }, { status: 404 });

  if (!fromThisSite(request)) {
    return Response.json({ outcome: 'refused', message: definition.wording.refused }, { status: 403 });
  }
  const length = request.headers.get('content-length');
  if (length === null || !/^\d+$/.test(length)) {
    return Response.json({ outcome: 'refused', message: definition.wording.refused }, { status: 411 });
  }
  if (Number(length) > REQUEST_BYTES_LIMIT) {
    return Response.json({ outcome: 'refused', message: definition.wording.refused }, { status: 413 });
  }
  ...
  try {
    ...
    return Response.json({ outcome: 'failed', message: definition.wording.failed }, { status: 400 });
  }

  return Response.json(await submit(definition, data, { address: clientAddress(request.headers) }, after));
}
