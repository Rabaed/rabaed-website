...
export async function generateMetadata(): Promise<Metadata> {
  const [{ meta }, locales] = await Promise.all([getPartnershipPage('ar'), publishedLocales(getPartnershipPage)]);
...
export default async function ArabicPartnershipPage() {
  const [content, locales] = await Promise.all([getPartnershipPage('ar'), publishedLocales(getPartnershipPage)]);
