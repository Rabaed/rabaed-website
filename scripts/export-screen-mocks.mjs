 ...
 *
 * Each render is written twice: the whole screen, and its Phone crop — the
 * box `phoneCropBox` names in `registry.ts`, cut from that same render, for
 * phones to show in its place (ticket 78, ADR-0022). Cut here rather than
 * drawn separately, so a crop can never show a screen its whole picture
 * no longer does.
 *
...
const manifestFile = path.join(repoRoot, 'src', 'screen-mocks', 'exported.json');
...
async function main() {
  const { SCREEN_MOCKS, phoneCropBox } = await import('../src/screen-mocks/registry.ts');
    ...
    for (const locale of locales) {
      for (const mock of SCREEN_MOCKS) {
        ...
        rendered.push({
          ...
          // What this picture was made from. The test that reads it back is
          // the one thing standing between an edited label and a site still
          // serving last month's picture of it.
          source: createHash('sha256').update(source).digest('hex'),
          phoneCrop,
          webp: await sharp(png).webp({ lossless: true, effort: 6 }).toBuffer(),
          phoneWebp: await sharp(png)
            .extract({ left: phoneCrop.x, top: phoneCrop.y, width: phoneCrop.width, height: phoneCrop.height })
  ...
  for (const locale of locales) {
    ...
    await mkdir(path.join(outDir, locale, 'phone'), { recursive: true });
  }
  for (const image of rendered) {
    await writeFile(path.join(outDir, image.locale, `${image.id}.webp`), image.webp);
    await writeFile(path.join(outDir, image.locale, 'phone', `${image.id}.webp`), image.phoneWebp);
    console.log(
      `${image.locale}/${image.id}`.padEnd(30),
      `${image.width}×${image.height}`.padEnd(12),
      `${Math.round(image.webp.length / 1024)} KB`.padEnd(8),
      `phone ${image.phoneCrop.width}×${image.phoneCrop.height}`.padEnd(18),
      `${Math.round(image.phoneWebp.length / 1024)} KB`,
    );
  }
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        note: 'Generated by scripts/export-screen-mocks.mjs — do not edit by hand.',
        images: rendered.map(({ webp, phoneWebp, ...record }) => record),
      },
      null,
      2,
    ) + '\n',
  ...
  console.log(`\nWrote ${path.relative(repoRoot, manifestFile)}`);
}
