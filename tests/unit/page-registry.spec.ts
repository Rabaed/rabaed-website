...
/** Every page the registry lists, by its Arabic address, and whether it has addresses beneath it. */
const PAGES = [
  ...MARKETING_PAGE_KEYS.map((key) => ({ path: MARKETING_PAGES[key].path, beneath: false })),
  ...Object.values(SITE_PAGES).map((page) => ({ path: page.path, beneath: page.beneath })),
];
...
const under = (address: string, prefix: string) => address === prefix || address.startsWith(`${prefix}/`);

/** What the registry says an address is, or `null` where it says nothing. */
function registered(address: string): string | null {
  const page = PAGES.find((each) => inArabic(address) === each.path || (each.beneath && under(inArabic(address), each.path)));
...
test('every route of the site is a page the registry lists, or named there as not one', async () => {
  const unregistered = (await appRoutes()).filter((route) => registered(route.address) === null);
  expect(
...
test('every page the registry lists has its route, and every marketing page its English route', async () => {
  const addresses = new Set((await appRoutes()).map((route) => route.address));
  ...
  for (const key of MARKETING_PAGE_KEYS) {
    const english = MARKETING_PAGES[key].path === '/' ? '/en' : `/en${MARKETING_PAGES[key].path}`;
...
test('every name for a route that is not a page still names a route', async () => {
  const addresses = (await appRoutes()).map((route) => route.address);
